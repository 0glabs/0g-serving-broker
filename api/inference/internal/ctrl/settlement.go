...
func (c *Ctrl) SettleFees(ctx context.Context) error {
	...
	if err != nil {
		c.logger.Error("Error marshalling settlement infos:", err)
		settlementInfoJSON = []byte("[]")
	}
	c.logger.Info("Settlement infos: ", string(settlementInfoJSON))
...
func (c Ctrl) ProcessSettlement(ctx context.Context) error {
	...
	c.logger.Info("Accounts at risk of having insufficient funds and will be settled immediately.")
