# 0G Compute Network Fine-tuning
...
1. Providers start their servers in a CVM and publish their fine-tuning services on the 0G Compute Network, including quotas, prices, and other information. When a server starts, it generates a signing key pair. The `/quoto` endpoint supports the RA report downloading so that customer can verify the server and get the signing public key (signing address).
2. **Service** **Verification**：Customers select an appropriate provider to use their `/quoto` interface to download the RA report, verify it against the specified contract, and save the signing address included in the RA report.
...
5. **Script Execution and Encryption**: The provider runs the script prepared by us and generates a TEE signature using the signning key, ensuring the following processes are executed correctly:
    - Verify that the signning address in contract customer account aligns with the signning address stored in the local database.
    ...
    - Encrypt the model using customer's public key.
    - Upload the encrypted model to 0G Storage and obtain the model root hash.
    - Generate a TEE signature of settlement metadata (model root hash, task fee, customer address, and nonce) using signing key.
    - **Settlement**: Use the TEE signature, model root hash, task fee, customer address, and nonce for settlement.
6. **Contract Settlement and Verification**:
...
7. **Model Decryption and Retrieval**:
...
*note*: The fine-tuning process is similar to inference but involves different internal logic, hence components and contracts are not reused.
...
2. Dataset size
3. Number of epochs
4. GPU type ( price decided by provider )
...
| /query-progress | GET | Queries the progress of a task, accepts parameters including job-id |
...
*note*: The fine-tuning service automatically registers to the contract when the broker starts.
